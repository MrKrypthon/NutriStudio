  ...
  return <AppChrome active="Pacientes" setActive={setActive}><div className="content clinical-content">
    ...
    {loadState === 'ready' && <>
      ...
      : tab === 'Diagnóstico' ? <div className="panel diagnosis-panel">
        ...
        <div className="form-card"><h3>Evaluación CESIVA</h3><p className="muted cesiva-hint">Evalúa cada criterio de la alimentación actual de {patientName}.</p><div className="cesiva-table">
          <div className="cesiva-row cesiva-head"><span>Criterio</span><b>Evaluación</b><b>Comentario</b></div>
          {CESIVA.map(([criterion, description]) => { const key = `CESIVA: ${criterion}`; const raw = currentValues[key]; const value = raw === true ? 'Cumple' : (typeof raw === 'string' ? raw : ''); const noteKey = `CESIVA: ${criterion} (comentario)`; return <div className="cesiva-row" key={criterion}><span><b>{criterion}</b><small>{description}</small></span><select value={value} onChange={(e) => updateField(key, e.target.value)}><option value="">—</option><option value="Cumple">Cumple</option><option value="Parcial">Parcial</option><option value="No cumple">No cumple</option></select><input value={currentValues[noteKey] ?? ''} onChange={(e) => updateField(noteKey, e.target.value)} placeholder="Comentario" /></div> })}
        </div></div>
